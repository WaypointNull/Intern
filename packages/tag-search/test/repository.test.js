...
const { createTagListRepository, createRetrievalIndex, parseCsvRecords } = require('..');
...
test('loadFromRecords applies an injected junk-token set', () => {
  const repo = createTagListRepository({ junkTokens: new Set(['blue_hair']) });
  const summary = repo.loadFromRecords(parseCsvRecords(CSV));
  assert.equal(summary.tags, 4);
  assert.ok(!repo.getTagSet().has('blue_hair'));
});
